...
/**
 * Парсит plans.md и возвращает структурированные задачи
 * @param {string} planContent - содержимое plans.md
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (inTodoSection) {
      ...
      if (completedMatch) {
        ...
        const taskTitle = completedMatch[3]
          .trim()
          .replace(/\s*✅\s*\*\*ВЫПОЛНЕНО\*\*.*$/, '')
          .trim()
        const task = {
          number: taskNumber,
          title: taskTitle,
      ...
      if (
        line.includes('#### КОД') ||
        line.includes('#### ФУНКЦИОНАЛ') ||
        line.includes('#### ВИЗУАЛ')
      ) {
...
/**
 * Загружает plans.md и парсит его
