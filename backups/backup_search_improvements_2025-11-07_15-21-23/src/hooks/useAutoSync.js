...
export function useAutoSync(options = {}) {
  ...
  const { entries, addEntry, updateEntry, deleteEntry, clearEntries, importEntries } = useEntriesStore();
  const { theme, categories, setTheme, updateSettings } = useSettingsStore();
   ...
   */
  const createBackup = useCallback((type = 'auto') => {
    const backup = {
      id: Date.now(),
      type,
   ...
   */
  const saveBackup = useCallback((backup) => {
    try {
   ...
   */
  const restoreFromBackup = useCallback((backupId) => {
    try {
   ...
   * @param {string} type - тип резервной копии
   */
  const createManualBackup = useCallback((type = 'manual') => {
   ...
   */
  const cleanupOldBackups = useCallback((olderThan = 7 * 24 * 60 * 60 * 1000) => { // 7 дней
    try {
   ...
   */
  const importBackups = useCallback((jsonData) => {
    try {
