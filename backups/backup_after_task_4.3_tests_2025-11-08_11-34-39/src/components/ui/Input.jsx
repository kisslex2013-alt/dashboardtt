    ...
    <div className="mb-4">
      ...
      <div className="relative">
        ...
        <input
          ...
          aria-invalid={!!error}
          aria-describedby={error ? `${label ? label.toLowerCase().replace(/\s+/g, '-') : 'input'}-error` : undefined}
          id={label ? `${label.toLowerCase().replace(/\s+/g, '-')}-input` : undefined}
        />
      </div>
      {error && (
        <p id={label ? `${label.toLowerCase().replace(/\s+/g, '-')}-error` : 'input-error'} className="text-red-500 text-sm mt-1" role="alert" aria-live="polite">{error}</p>
      )}
    </div>
  );
});
