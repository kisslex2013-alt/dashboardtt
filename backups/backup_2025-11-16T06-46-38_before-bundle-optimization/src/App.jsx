...
const EntriesList = lazy(() =>
  import('./components/entries/EntriesList').then(module => ({ default: module.EntriesList }))
)
const PomodoroPanel = lazy(() =>
  import('./components/pomodoro/PomodoroPanel').then(module => ({ default: module.PomodoroPanel }))
)
...
const PaymentDatesSettingsModal = lazy(() =>
  import('./components/modals/PaymentDatesSettingsModal/index.jsx').then(module => ({
...
function App() {
  ...
  const handleOpenCategories = useCallback(() => {
    window.dispatchEvent(new CustomEvent('open-categories-modal'))
  }, [])
  
  const handleOpenBackups = useCallback(() => {
    window.dispatchEvent(new CustomEvent('open-backups-modal'))
  }, [])
  ...
  return (
    <>
      ...
      <div 
        ...
        tabIndex={-1}
      >
        {/* ✅ ОПТИМИЗАЦИЯ: Lazy loading для StatisticsOverview */}
        <Suspense fallback={<div className="mb-6 glass-effect rounded-xl p-8"><div className="flex items-center justify-center py-4"><div className="w-8 h-8 border-4 border-blue-500 border-t-transparent rounded-full animate-spin"></div></div></div>}>
          <StatisticsOverview />
        </Suspense>
        
        {/* ✅ ОПТИМИЗАЦИЯ: Lazy loading для Pomodoro панели (если включена) */}
        {pomodoroSettings?.enabled && (
          <Suspense fallback={<div className="mb-6 glass-effect rounded-xl p-4"><div className="flex items-center justify-center py-2"><div className="w-5 h-5 border-4 border-blue-500 border-t-transparent rounded-full animate-spin"></div></div></div>}>
            <PomodoroPanel />
          </Suspense>
        )}
        ...
        {/* ✅ ОПТИМИЗАЦИЯ: Lazy loading для EntriesList */}
        <Suspense fallback={<div className="glass-effect rounded-xl p-6"><div className="flex items-center justify-center py-8"><div className="w-8 h-8 border-4 border-blue-500 border-t-transparent rounded-full animate-spin"></div></div></div>}>
          <EntriesList
