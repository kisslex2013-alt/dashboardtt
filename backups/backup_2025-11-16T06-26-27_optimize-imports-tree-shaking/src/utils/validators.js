...
export function validateSettings(settings) {
  ...
  if (settings.notifications) {
    if (
      ...
      (isNaN(settings.notifications.volume) ||
...
export function validateSettingsForm(formData) {
  ...
  if (
    ...
    (isNaN(formData.notificationVolume) ||
      formData.notificationVolume < 0 ||
      formData.notificationVolume > 100)
  ) {
  ...
  if (!Array.isArray(importData.data.entries)) {
    ...
    importData.data.entries.forEach((entry, index) => {
      ...
      if (!entryValidation.isValid) {
        errors[`entry_${index}`] =
          `Запись ${index + 1}: ${Object.values(entryValidation.errors).join(', ')}`
      }
      if (Object.keys(entryValidation.warnings).length > 0) {
        warnings[`entry_${index}`] =
          `Запись ${index + 1}: ${Object.values(entryValidation.warnings).join(', ')}`
      }
    })
  }
    ...
    importData.data.categories.forEach((category, index) => {
      ...
      if (!categoryValidation.isValid) {
        errors[`category_${index}`] =
          `Категория ${index + 1}: ${Object.values(categoryValidation.errors).join(', ')}`
      }
      if (Object.keys(categoryValidation.warnings).length > 0) {
        warnings[`category_${index}`] =
          `Категория ${index + 1}: ${Object.values(categoryValidation.warnings).join(', ')}`
      }
    })
  }
