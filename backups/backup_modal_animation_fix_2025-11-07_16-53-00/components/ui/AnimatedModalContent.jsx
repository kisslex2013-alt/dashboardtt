...
export function AnimatedModalContent({ 
  ...
  useEffect(() => {
    ...
    // ✨ ИСПРАВЛЕНИЕ: Пропускаем анимацию при первом рендере (когда previousKeyRef.current === undefined/null)
    if (previousKeyRef.current === undefined || previousKeyRef.current === null) {
      setDisplayedContent(children);
      previousKeyRef.current = contentKey;
      return;
    }
