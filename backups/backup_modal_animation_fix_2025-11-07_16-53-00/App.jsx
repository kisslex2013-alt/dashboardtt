...
function App() {
  ...
  // ✨ ИСПРАВЛЕНИЕ: Убрали useDelayedUnmount для модальных окон с HeadlessUI transitions
  // HeadlessUI сам управляет жизненным циклом через unmount prop и transition
  // useDelayedUnmount создавал задвоение и мерцание модальных окон
  ...
  const handleClearDemoData = useCallback(() => {
    try {
      ...
      showError('Ошибка при удалении тестовых данных: ' + error.message);
    }
    ...
    const loadDemo = async () => {
      ...
      if (entries.length === 0 && !localStorage.getItem('demo_data_loaded')) {
        try {
          ...
          showError('Не удалось загрузить тестовые данные: ' + error.message);
        }
      }
    };
  ...
  return (
    <div className="min-h-screen bg-gray-100 dark:bg-gray-900 transition-colors duration-300">
      <div className="max-w-7xl mx-auto p-6">
        ...
        {/* ✨ ИСПРАВЛЕНИЕ: HeadlessUI transitions управляют жизненным циклом, useDelayedUnmount не нужен */}
