...
export function PlanFactCompactView({ shouldAnimate = true, shouldShow = true }) {
  // ✅ ИСПРАВЛЕНИЕ: Используем селектор для получения только entries
  // Это предотвратит ненужные перерендеры при изменении других полей store
  ...
  const planFactData = useMemo(() => {
    ...
    const calculateFact = (filter) => {
      ...
      const sum = filtered.reduce((sum, e) => {
        ...
        const numValue = typeof earned === 'string' 
  ...
  }, [paymentDates, entries, dailyPlanValue, currentYear, currentMonth, workScheduleTemplate, workScheduleStartDay, customWorkDates]);
        ...
        >
          <div className="relative z-10">
            ...
            <div className="space-y-4">
              {/* День */}
              <div className={shouldAnimate ? 'animate-slide-up' : ''} style={shouldAnimate ? { animationDelay: '0.1s', animationFillMode: 'both' } : {}}>
                <div className="flex justify-between items-baseline">
                  <p className={`text-gray-500 dark:text-gray-400 text-sm ${shouldAnimate ? 'opacity-0 animate-fade-in' : ''}`} style={shouldAnimate ? { animationDelay: '0.15s', animationFillMode: 'forwards' } : {}}>День</p>
                  <AnimatedCounter
                    value={day}
                    suffix=" ₽"
                    decimals={0}
                ...
                <div className="w-full bg-gray-300 dark:bg-gray-700/50 rounded-full h-1 mt-1 overflow-hidden">
                  <div 
                    ...
                    style={{ 
                      ...
                      animation: shouldAnimate ? `slideInProgress ${progressBarSpeeds.day}s cubic-bezier(0.4, 0, 0.2, 1) 0.25s both` : 'none'
                    }}
                  />
                </div>
                <p className={`text-right text-xs text-gray-700 dark:text-gray-300 font-medium mt-1 ${shouldAnimate ? 'opacity-0 animate-fade-in' : ''}`} style={shouldAnimate ? { animationDelay: '0.3s', animationFillMode: 'forwards' } : {}}>
                  План: {dailyPlanValue.toLocaleString('ru-RU')} ₽
                </p>
              </div>

              {/* Месяц */}
              <div className={shouldAnimate ? 'animate-slide-up' : ''} style={shouldAnimate ? { animationDelay: '0.15s', animationFillMode: 'both' } : {}}>
                <div className="flex justify-between items-baseline">
                  <p className={`text-gray-500 dark:text-gray-400 text-sm ${shouldAnimate ? 'opacity-0 animate-fade-in' : ''}`} style={shouldAnimate ? { animationDelay: '0.2s', animationFillMode: 'forwards' } : {}}>Месяц</p>
                  <AnimatedCounter
                    value={month}
                    suffix=" ₽"
                    decimals={0}
                ...
                <div className="w-full bg-gray-300 dark:bg-gray-700/50 rounded-full h-1 mt-1 overflow-hidden">
                  <div 
                    ...
                    style={{ 
                      ...
                      animation: shouldAnimate ? `slideInProgress ${progressBarSpeeds.month}s cubic-bezier(0.4, 0, 0.2, 1) 0.3s both` : 'none'
                    }}
                  />
                </div>
                <p className={`text-right text-xs text-gray-700 dark:text-gray-300 font-medium mt-1 ${shouldAnimate ? 'opacity-0 animate-fade-in' : ''}`} style={shouldAnimate ? { animationDelay: '0.35s', animationFillMode: 'forwards' } : {}}>
            ...
            {paymentData.length === 0 ? (
              ...
              <div className="space-y-4">
                {paymentData.map((payment, index) => (
                  ...
                  >
                    <div className="flex justify-between items-baseline">
                      <p className={`text-gray-500 dark:text-gray-400 text-sm ${shouldAnimate ? 'opacity-0 animate-fade-in' : ''}`} style={shouldAnimate ? { animationDelay: `${0.25 + index * 0.05}s`, animationFillMode: 'forwards' } : {}}>
                        {payment.name}{' '}
                      ...
                      <AnimatedCounter
                        ...
                        decimals={0}
                        className={`text-xl font-bold text-gray-900 dark:text-white ${shouldAnimate ? 'opacity-0 animate-fade-in' : ''}`}
                        style={shouldAnimate ? { animationDelay: `${0.3 + index * 0.05}s`, animationFillMode: 'forwards' } : {}}
                        resetAnimation={shouldAnimate}
                      />
                    </div>
                    <div className="w-full bg-gray-300 dark:bg-gray-700/50 rounded-full h-1 mt-1 overflow-hidden">
                    ...
                    <p className={`text-right text-xs text-gray-700 dark:text-gray-300 font-medium mt-1 ${shouldAnimate ? 'opacity-0 animate-fade-in' : ''}`} style={shouldAnimate ? { animationDelay: `${0.4 + index * 0.05}s`, animationFillMode: 'forwards' } : {}}>
        ...
        >
          <div className="relative z-10">
            ...
            <div className="space-y-6 flex flex-col justify-center h-full">
              {/* Год */}
              <div className={`border-l-4 border-orange-400 pl-4 ${shouldAnimate ? 'animate-slide-up opacity-0 animate-fade-in' : ''}`} style={shouldAnimate ? { animationDelay: '0.3s', animationFillMode: 'both' } : {}}>
                <p className={`text-gray-500 dark:text-gray-400 text-sm ${shouldAnimate ? 'opacity-0 animate-fade-in' : ''}`} style={shouldAnimate ? { animationDelay: '0.35s', animationFillMode: 'forwards' } : {}}>Год</p>
                <AnimatedCounter
                  value={year}
                  suffix=" ₽"
                  decimals={0}
                  className={`text-3xl font-bold text-gray-900 dark:text-white ${shouldAnimate ? 'opacity-0 animate-fade-in' : ''}`}
                  style={shouldAnimate ? { animationDelay: '0.4s', animationFillMode: 'forwards' } : {}}
                  resetAnimation={shouldAnimate}
                />
              </div>

              {/* За все время */}
              <div className={`border-l-4 border-red-400 pl-4 ${shouldAnimate ? 'animate-slide-up opacity-0 animate-fade-in' : ''}`} style={shouldAnimate ? { animationDelay: '0.35s', animationFillMode: 'both' } : {}}>
                <p className={`text-gray-500 dark:text-gray-400 text-sm ${shouldAnimate ? 'opacity-0 animate-fade-in' : ''}`} style={shouldAnimate ? { animationDelay: '0.4s', animationFillMode: 'forwards' } : {}}>За все время</p>
                <AnimatedCounter
                  value={allTime}
                  suffix=" ₽"
                  decimals={0}
                  className={`text-3xl font-bold text-gray-900 dark:text-white ${shouldAnimate ? 'opacity-0 animate-fade-in' : ''}`}
                  style={shouldAnimate ? { animationDelay: '0.45s', animationFillMode: 'forwards' } : {}}
                  resetAnimation={shouldAnimate}
                />
              </div>
            </div>
