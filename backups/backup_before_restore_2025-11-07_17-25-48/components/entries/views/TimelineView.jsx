...
export function TimelineView({ entries, onEdit, selectionMode = false, selectedEntries = new Set(), onToggleSelection }) {
  ...
  // ✨ ОПТИМИЗАЦИЯ: Инкрементальная загрузка - показываем по 40 дней
  ...
  const handleLoadMore = useCallback(() => {
    requestAnimationFrame(() => {
      setVisibleCount(prev => Math.min(prev + 40, sortedDates.length));
    });
  }, [sortedDates.length]);
      ...
      <div className="space-y-6">
                ...
                <div className="bg-gradient-to-r from-blue-50 to-blue-100 dark:from-blue-900/30 dark:to-blue-800/30 p-4">
                  ...
                  <div className="grid grid-cols-3 gap-2 pt-2 border-t border-gray-200 dark:border-gray-700">
                    <div className="flex flex-col items-center">
                      ...
                      <strong className="text-sm font-mono text-gray-800 dark:text-white">{metrics.longestSession}</strong>
                    </div>
                    <div className="flex flex-col items-center">
                      <div className="flex items-center gap-1 text-xs text-gray-600 dark:text-gray-400 mb-1">
                      ...
                      <strong className="text-sm font-mono text-gray-800 dark:text-white">{metrics.longestBreak}</strong>
                    </div>
                    <div className="flex flex-col items-center">
                      <div className="flex items-center gap-1 text-xs text-gray-600 dark:text-gray-400 mb-1">
                      ...
                      <strong className="text-sm font-mono text-gray-800 dark:text-white">{metrics.averageRate}</strong>
                    </div>
                  </div>
                </div>
        ...
        {hasMore && (
          <div className="flex justify-center py-4 mt-6">
            ...
            >
              Показать еще {Math.min(40, remainingCount)} дней (осталось {remainingCount})
            </button>
          </div>
        )}
      </div>
