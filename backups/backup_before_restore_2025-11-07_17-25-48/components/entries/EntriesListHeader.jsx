...
import { Undo, Redo, List, Grid, Clock, Plus, Play, Square, Download, Upload, Folder, HardDrive, CheckSquare, X, Search, ArrowUp } from 'lucide-react';
...
export function EntriesListHeader({
  ...
  searchQuery,
  onSearchChange,
  onSearchExpandedChange // ✨ Новый callback для передачи состояния открытия поиска
}) {
  ...
  // ✨ УЛУЧШЕНИЕ: Уведомляем родителя об изменении состояния поиска
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <div className="relative">
              {!isSearchExpanded ? (
                <button
                  onClick={() => {
                    handleSearchExpandedChange(true);
            ...
            <button
              ...
              aria-label="Таймер"
              style={{ width: 'auto' }}
              data-icon-id={timer.isRunning ? "header-timer-stop" : "header-timer-start"}
            >
              {timer.isRunning ? (
                <>
            ...
            <button
              ...
              style={{ width: 'auto' }}
              data-icon-id="header-categories"
            >
          ...
          <input
            ...
            onBlur={(e) => {
              if (!e.target.value) {
                setIsSearchFocused(false);
                setTimeout(() => {
                  if (!e.target.value) handleSearchExpandedChange(false);
                }, 300);
              }
            }}
            ...
            style={{
              animation: isClosing 
                ? 'collapseWidth 0.3s ease-in' 
                : isSearchFocused 
                ? 'expandWidth 0.3s ease-out' 
                : 'none'
            }}
          />
          <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 w-4 h-4 text-gray-400 pointer-events-none" />
          
          {/* ✨ Кнопка справа: крестик (если есть текст) или стрелка вверх (если пусто) */}
          <div className="absolute right-3 top-1/2 transform -translate-y-1/2">
            {searchQuery ? (
              ...
              <button
                ...
                className="w-5 h-5 flex items-center justify-center text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 transition-colors rounded-full hover:bg-gray-200 dark:hover:bg-gray-700"
              ...
              // Стрелка вверх для закрытия поля поиска
              <button
                onClick={(e) => {
                  e.stopPropagation();
                  handleSearchExpandedChange(false);
                }}
                className="w-5 h-5 flex items-center justify-center text-gray-400 hover:text-blue-500 dark:hover:text-blue-400 transition-all duration-300 rounded-full hover:bg-gray-200 dark:hover:bg-gray-700 hover:scale-110"
                title="Закрыть поиск"
