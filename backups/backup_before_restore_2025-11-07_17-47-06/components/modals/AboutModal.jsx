import { History, Rocket, Heart, Copy, Check, Code, Loader2, ChevronDown, ChevronUp } from 'lucide-react';
...
export function AboutModal({ isOpen, onClose }) {
  ...
  const getTotalTasks = (section) => {
    ...
    return getTaskCount(section, 'completed') + 
  ...
  const technologies = [
    ...
    {
      name: 'Framer Motion',
      description: 'Библиотека для создания плавных анимаций в React. Используется для анимации чисел, текста, дат и других элементов интерфейса.',
      version: '12.23+',
      icon: '🎬',
    },
  ];
  ...
  // Приоритет: 1) Статическое изображение из public/images/, 2) Автоматическая генерация через API
  const getQRCodeUrl = () => {
    // ВАРИАНТ 1: Использовать статическое изображение QR кода (если загружено в public/images/)
    // Если вы загрузили файл qr-code-alexey.png в папку public/images/, раскомментируйте строку ниже:
    return '/images/qr-code-alexey.png';
    
    // ВАРИАНТ 2: Автоматическая генерация QR кода через API (используется по умолчанию)
            ...
            {isLoadingChangelog ? (
              ...
              changelogData.map((item, index) => {
                ...
                const categoryColors = {
                  ...
                  'Улучшения интерфейса': { color: 'purple', text: 'text-purple-600 dark:text-purple-400' },
                  'Исправления ошибок': { color: 'red', text: 'text-red-600 dark:text-red-400' },
                  'Технические улучшения': { color: 'teal', text: 'text-teal-600 dark:text-teal-400' }
                };
                
                return (
                  <div key={index} className="glass-effect rounded-lg p-4 border border-gray-200/50 dark:border-gray-700/50 hover:border-blue-300 dark:hover:border-blue-600 hover:shadow-md transition-all duration-200">
                    ...
                    <AnimatePresence>
                      {isExpanded && (
                        ...
                        {Object.entries(item.categories).map(([categoryName, items]) => {
                          ...
                          const categoryInfo = categoryColors[categoryName] || { color: 'blue', text: 'text-blue-600 dark:text-blue-400' };
                              ...
                              <div className="flex items-center gap-2 mb-2">
                                <span className={`text-xs font-semibold ${categoryInfo.text}`}>
                                  {categoryName === 'Новые возможности' ? '✨' : 
                                   ...
                                   categoryName === 'Технические улучшения' ? '⚙️' : '📝'} {categoryName === 'Новые возможности' ? 'Новые' : 
                                   categoryName === 'Улучшения интерфейса' ? 'Интерфейс' : 
                                   categoryName === 'Исправления ошибок' ? 'Исправления' : 
                                   categoryName === 'Технические улучшения' ? 'Технические' : categoryName} ({getCategoryCount(versionKey, categoryName)})
                                </span>
                              </div>
                              <div className="pl-4 space-y-1 text-xs text-gray-600 dark:text-gray-400">
                                {items.map((change, changeIndex) => {
                                  if (typeof change === 'object' && change !== null) {
                                    return (
                                      <div key={changeIndex}>• {change.text}</div>
                                    );
                                  }
                                  const emojiMatch = typeof change === 'string' ? change.match(/^([\u{1F300}-\u{1F9FF}]|[\u{2600}-\u{26FF}]|[\u{2700}-\u{27BF}])/u) : null;
                                  const emoji = emojiMatch ? emojiMatch[0] : null;
                                  const text = typeof change === 'string' 
                                    ? (emoji ? change.substring(emoji.length).trim() : change).replace(/^\*\*[^*]+\*\*\s*-\s*/, '')
                                    : String(change);
                                  return (
                                    <div key={changeIndex}>• {text}</div>
                                  );
                                })}
                              </div>
                            </div>
          ...
          {activeTab === 'support' && (
            ...
            <div className="grid grid-cols-3 gap-4">
              ...
              {bankCards.map((card, index) => {
                ...
                const formattedCardNumber = card.cardNumber.replace(/\s/g, '').match(/.{1,4}/g)?.join(' ') || card.cardNumber;
