...
export function CalendarHeatmap({ entries }) {
  const { theme, workScheduleTemplate, workScheduleStartDay, customWorkDates, dailyGoal } = useSettingsStore();
  ...
  const findDayByPosition = useMemo(() => {
    return (targetDate, positionIndex) => {
      ...
      if (positionIndex === undefined || positionIndex === null || positionIndex < 0 || positionIndex >= monthDays.length) {
        return null;
      }
    ...
    const updateTooltipPositions = () => {
      ...
      if (tooltipCompareRef.current && hoveredDayCompare) {
        ...
        if (compareCalendarElement) {
          const dayElement = compareCalendarElement.querySelector(`[data-day-index="${hoveredDayCompare.positionIndex}"]`);
      ...
      if (tooltipRef.current && hoveredDay && hoveredDayCompare) {
        ...
        if (isHoveringCompare) {
          ...
          if (currentCalendarElement) {
            const dayElement = currentCalendarElement.querySelector(`[data-day-index="${hoveredDay.positionIndex}"]`);
            if (dayElement) {
              const dayRect = dayElement.getBoundingClientRect();
  ...
  const renderCalendar = (date, setDate, title) => {
    ...
    const highlightedDay = title === 'Текущий период' 
      ? hoveredDayCompare 
      : hoveredDay;
