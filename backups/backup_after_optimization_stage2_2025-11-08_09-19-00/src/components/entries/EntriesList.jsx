...
export function EntriesList({ onAddNew, onStartTimer, onEditEntry, onUndo, onRedo, canUndo = false, canRedo = false, onExport, onImport }) {
  ...
  // ✅ ОПТИМИЗАЦИЯ: Мемоизированная функция для получения названия категории
  ...
  const filterByDate = useCallback((entry) => {
  ...
  const handleBulkCategoryConfirm = useCallback((categoryId) => {
  ...
  const handleBulkExport = useCallback(async () => {
    ...
    try {
      ...
      const errorMessage = handleError(error, { operation: 'Экспорт записей', count: selectedIds.length });
