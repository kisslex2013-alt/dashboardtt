...
// import PropTypes from 'prop-types'; // Временно отключено для отладки
  ...
  const handleFileSelect = async (event) => {
    ...
    try {
      ...
      reader.onerror = (error) => {
        // ИСПРАВЛЕНО: Используем централизованную обработку ошибок
        const errorMessage = handleError(new Error('Не удалось прочитать файл'), { operation: 'Чтение файла' });
        setError(errorMessage);
        setIsLoading(false);
      };
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      await new Promise((resolve, reject) => {
        ...
        reader.onerror = (error) => {
          // ИСПРАВЛЕНО: Используем централизованную обработку ошибок
          const errorMessage = handleError(new Error('Не удалось прочитать файл'), { operation: 'Чтение файла для импорта' });
          setError(errorMessage);
      ...
      // ИСПРАВЛЕНО: Используем централизованную обработку ошибок
      const errorMessage = handleError(err, { operation: 'Импорт данных (общая ошибка)', mode: importMode });
      setError(errorMessage);
    } finally {
      setIsLoading(false);
    }
