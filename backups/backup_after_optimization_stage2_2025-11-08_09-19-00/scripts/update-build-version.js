...
// Формат: build hh:mm_dd/mm/yy
const buildVersion = `build ${hours}:${minutes}_${day}/${month}/${year}`;
...
try {
  ...
  // Ищем паттерн: Time Tracker Dashboard v1.1.0 или Time Tracker Dashboard v1.1.0 build ...
  const versionPattern = /(Time Tracker Dashboard v\d+\.\d+\.\d+)(\s+build\s+[\d:/\s_]+)?/;
  
  if (versionPattern.test(content)) {
    // Заменяем версию
    content = content.replace(
      versionPattern,
      `Time Tracker Dashboard v1.1.0 ${buildVersion}`
    );
