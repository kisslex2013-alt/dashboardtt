 ...
 */
export const ScheduleTemplateCard = memo(function ScheduleTemplateCard({
  template,
  isSelected,
  onSelect,
  onCustomDayToggle,
  ...
  return (
    <div
      ...
      className={`dashboard-card p-3 sm:p-4 border-2 rounded-xl bg-white dark:bg-gray-800 ${
        isSelected 
    ...
    >
      ...
      <div className="flex items-start gap-2 sm:gap-3 mb-2 sm:mb-3">
        {/* Icon */}
        <div className={`w-10 h-10 sm:w-12 sm:h-12 rounded-lg bg-gradient-to-br ${getIconColorClasses(template.iconColor)} flex items-center justify-center flex-shrink-0`}>
      ...
      <div className="grid grid-cols-7 gap-0.5 mb-2 sm:mb-3">
        ...
        {(template.id === 'custom' ? ['П', 'В', 'С', 'Ч', 'П', 'С', 'В'] : template.weekDays).map((dayName, idx) => (
          <div key={`header-${idx}`} className="text-[8px] text-center text-gray-500 dark:text-gray-400 font-medium pb-0.5">
            {dayName}
          </div>
        ))}
          ...
          return (
            <button
              ...
              className={`day-dot w-4 h-4 sm:w-5 sm:h-5 text-[9px] sm:text-[10px] flex items-center justify-center ${
                ...
                template.id === 'custom' ? 'cursor-pointer hover-lift-scale click-shrink' : 'cursor-default'
              } transition-normal`}
            >
              {calendarItem.day}
            </button>
