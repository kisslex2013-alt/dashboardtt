...
import { IdealDayChart } from '../charts/IdealDayChart';
import { ForecastChart } from '../charts/ForecastChart';
import { CalendarHeatmap } from '../charts/CalendarHeatmap';
import { CategoryEfficiencyChart } from '../charts/CategoryEfficiencyChart';
import { CombinedChart } from '../charts/CombinedChart';
import { ProductivityHoursChart } from '../charts/ProductivityHoursChart';
...
export function AnalyticsSection() {
  ...
  const chartLabels = {
    ...
    productivityHours: 'Продуктивность по часам',
  };
  ...
  return (
    ...
    <div className={`${isExpanded || (shouldMountContent && !isExitingContent) ? 'mb-6' : 'mb-0'} relative`}>
      ...
      <div className={`glass-effect rounded-xl p-4 mb-4 overflow-visible ${isExpanded ? 'sticky top-0 z-40 backdrop-blur-xl bg-white/90 dark:bg-gray-800/90 shadow-lg transition-normal' : ''}`}>
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            {isExpanded && (
              <div className="flex items-center gap-2">
                <span className="text-sm font-medium text-gray-600 dark:text-gray-400">Режим:</span>
          ...
          {isExpanded && (
            <div className="flex items-center gap-2">
              <span className="text-sm font-medium text-gray-600 dark:text-gray-400">Период:</span>
              ...
              <div className="relative">
                ...
                <button
                  ...
                  className="glass-effect px-4 py-2 pr-10 rounded-lg border border-gray-300 dark:border-gray-600 focus:outline-none focus:ring-2 focus:ring-blue-500 cursor-pointer font-medium text-sm min-w-[180px] text-left transition-normal hover-lift-scale click-shrink"
                >
                  {dateFilter}
                ...
                {shouldMountFilterDropdown && (
                  <div 
                    ref={dropdownRef}
                    className={`absolute right-0 mt-2 w-64 glass-effect rounded-lg border border-gray-300 dark:border-gray-600 shadow-xl z-[9999] backdrop-blur-lg bg-white/80 dark:bg-gray-800/80 ${
              ...
              <div className="relative">
                ...
                >
                  ...
                  <ChevronDownIcon className={`absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-500 transition-transform duration-200 ${isVisibilityMenuOpen ? 'rotate-180' : ''}`} />
                </button>
      ...
      {shouldMountContent && (
        ...
        >
          {/* Объединенный график в режиме combined */}
          {/* ИСПРАВЛЕНО: Убраны отдельные анимации fade-out - графики анимируются вместе с контейнером через slide-out */}
          {chartDisplay === 'combined' && (chartVisibility.dynamics || chartVisibility.rateDistribution) && (
            <CombinedChart
          ...
          {chartDisplay === 'separate' && (
            <>
              ...
              {chartVisibility.trends && (
                <TrendsChart entries={filteredEntries} />
              )}
            </>
          )}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            {chartVisibility.timeDistribution && (
              <TimeDistribution entries={filteredEntries} />
            )}
          </div>
          ...
          {/* Календарь доходов - во всю ширину (не зависит от общего фильтра, имеет свою навигацию) */}
