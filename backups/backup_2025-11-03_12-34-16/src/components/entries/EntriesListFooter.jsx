    ...
    <div className="flex flex-wrap items-center justify-between gap-2 mt-4 pt-4 border-t border-gray-200 dark:border-gray-700">
      <div className="flex items-center gap-3 flex-wrap">
        ...
        <div className={`transition-all duration-300 ease-in-out overflow-hidden ${
          ...
          {showDatePicker && (
            ...
            <div className="flex items-center gap-2 relative">
              <label className="text-sm font-medium text-gray-600 dark:text-gray-400 whitespace-nowrap">С даты:</label>
              <div className="relative">
                <input
                  ref={startDateInputRef}
                  type="text"
                  value={customDateRange.start ? customDateRange.start.split('-').reverse().join('/') : ''}
            ...
            <div className="flex items-center gap-2 relative">
              <label className="text-sm font-medium text-gray-600 dark:text-gray-400 whitespace-nowrap">По дату:</label>
              <div className="relative">
                <input
                  ref={endDateInputRef}
                  type="text"
                  value={customDateRange.end ? customDateRange.end.split('-').reverse().join('/') : ''}
        ...
        {filteredCount < entriesCount && (
          <span className="text-sm text-gray-600 dark:text-gray-400 whitespace-nowrap">
            Записей: <span className="font-semibold text-gray-700 dark:text-gray-300">{filteredCount}</span> из <span className="font-semibold text-gray-700 dark:text-gray-300">{entriesCount}</span>
          </span>
        )}
      </div>
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="relative" ref={dropdownContainerRef}>
          ...
          >
            {dateFilter}
            <ChevronDown className={`absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-500 transition-transform duration-200 ${isFilterDropdownOpen ? 'rotate-180' : ''}`} />
          </button>
          ...
          {shouldMountFilterDropdown && (
            <div 
              ref={dropdownMenuRef}
              className={`absolute right-0 bottom-full mb-2 w-64 glass-effect rounded-lg border border-gray-300 dark:border-gray-600 shadow-xl z-[9999] max-h-96 overflow-y-auto ${
