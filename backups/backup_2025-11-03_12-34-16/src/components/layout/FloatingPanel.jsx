...
import { Square, Maximize2, Minimize2, Settings } from 'lucide-react';
...
export function FloatingPanel() {
  ...
  const { openModal, showSuccess } = useUIStore();
  ...
  }, [isDragging, position, floatingPanel, updateSettings]);

  // Подписываемся на события мыши для drag & drop
  useEffect(() => {
    if (isDragging) {
  ...
  /**
   * Переключение темы панели
   */
  const toggleTheme = useCallback(() => {
    const themes = ['glass', 'solid', 'minimal'];
    const currentIndex = themes.indexOf(floatingPanel?.theme || 'glass');
  ...
  return (
    <div
      ...
      className={`
        ...
        ${isExpanded ? 'w-[320px] h-[180px]' : 'w-[200px] h-[100px]'}
    ...
    >
      <div className="flex flex-col h-full gap-2">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2 flex-1 min-w-0">
            ...
            <div className="flex flex-col min-w-0">
              ...
              <div className="text-xl font-mono font-bold">
                {formatElapsed(timer.getSeconds())}
              </div>
            </div>
          </div>
          ...
          <button
            onClick={toggleSize}
            className="floating-panel-button p-1 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700 transition-normal hover-lift-scale click-shrink flex-shrink-0"
        ...
        <div className="flex items-center justify-between mt-auto gap-2">
          ...
          <button
            ...
            className="floating-panel-button px-2 py-1.5 rounded-lg bg-red-500 hover:bg-red-600 text-white transition-normal hover-lift-scale click-shrink flex items-center gap-1.5 flex-1 min-w-0"
            title="Остановить таймер"
          >
            <Square className="w-3.5 h-3.5 flex-shrink-0" />
            <span className="text-xs font-medium truncate">Стоп</span>
          </button>

          {/* Кнопка переключения темы */}
          <button
            onClick={toggleTheme}
            className="floating-panel-button p-1.5 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700 transition-normal hover-lift-scale click-shrink flex-shrink-0"
            title={`Тема: ${theme} (клик для смены)`}
          >
