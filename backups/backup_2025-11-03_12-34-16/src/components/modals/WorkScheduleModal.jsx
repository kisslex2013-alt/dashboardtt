...
export function WorkScheduleModal({ isOpen, onClose }) {
  const { 
    ...
    customWorkDates: savedCustomWorkDates,
    updateSettings 
  } = useSettingsStore();
  ...
  return (
    <BaseModal
      ...
      title={
        <div>
          <div className="text-xl sm:text-2xl font-bold bg-gradient-to-r from-blue-600 to-blue-700 bg-clip-text text-transparent mb-1">
            Рабочий график
          </div>
          <p className="text-xs sm:text-sm text-gray-600 dark:text-gray-400 hidden sm:block">
            Выберите оптимальный режим работы
          </p>
        </div>
      }
      size="full"
