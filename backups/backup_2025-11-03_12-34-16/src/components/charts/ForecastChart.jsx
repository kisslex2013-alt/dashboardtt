...
import { format, parseISO, startOfMonth, endOfMonth, startOfYear, endOfYear, eachDayOfInterval, eachMonthOfInterval } from 'date-fns';
...
export function ForecastChart({ entries, dateFilter = 'month' }) {
  const { theme, dailyGoal } = useSettingsStore();
  ...
  const chartData = useMemo(() => {
    ...
    if (dateFilter === 'month') {
      ...
      // Рассчитываем месячный план (упрощенная версия - используем dailyGoal * 30)
      const monthlyPlan = validDailyPlan * 30;
      ...
      const data = Array.from({ length: 12 }, (_, i) => {
        const month = i;
        const monthName = format(new Date(year, month, 1), 'MMM', { locale: { localize: { month: (n) => ['Янв', 'Фев', 'Мар', 'Апр', 'Май', 'Июн', 'Июл', 'Авг', 'Сен', 'Окт', 'Ноя', 'Дек'][n] } } });
