...
export function exportToJSON(entries, categories, settings, options = {}) {
  return new Promise((resolve, reject) => {
    try {
      ...
      a.href = url;
      a.download = options.filename || `time-tracker-export-${new Date().toISOString().split('T')[0]}.json`;
...
export function exportToExcel(entries, categories) {
  return new Promise((resolve, reject) => {
    try {
      ...
      const a = document.createElement('a');
      a.href = url;
      a.download = `time-tracker-export-${new Date().toISOString().split('T')[0]}.xlsx`;
      a.style.display = 'none';
...
export function getExportStats(entries, categories) {
  ...
  return {
    ...
    dateRange: entries && entries.length > 0 ? {
 ...
 */
export function checkVersionCompatibility(importVersion, currentVersion = "1.0") {
