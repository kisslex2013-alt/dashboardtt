...
export function calculateEarningsTrend(entries) {
  ...
  recentEntries.forEach(entry => {
    ...
    weeks[3 - weekIndex].push(parseFloat(entry.earned) || 0);
  });
...
export function calculateTodayAnomaly(entries) {
  ...
  const todayCompletedEntries = entries.filter(entry => 
    entry.date === today && entry.end
  );
  ...
  const totalHistorical = historical.reduce((sum, entry) => 
    sum + (parseFloat(entry.earned) || 0), 0
  );
  ...
  const todayTotal = todayCompletedEntries.reduce((sum, entry) => 
    sum + (parseFloat(entry.earned) || 0), 0
  );
