...
export function validateEntryForm(formData) {
  ...
  if (!formData.start) {
    errors.start = 'Введите время начала';
  } else if (!isValidTime(formData.start)) {
    errors.start = 'Неверный формат времени';
  }
  
  // Проверяем время окончания
  if (!formData.end) {
    errors.end = 'Введите время окончания';
  } else if (!isValidTime(formData.end)) {
    errors.end = 'Неверный формат времени';
  }
  
  // Проверяем логику времени
  if (formData.start && formData.end && isValidTime(formData.start) && isValidTime(formData.end)) {
  ...
  if (formData.description && formData.description.length > 500) {
    warnings.longDescription = 'Описание слишком длинное (максимум 500 символов)';
  }
