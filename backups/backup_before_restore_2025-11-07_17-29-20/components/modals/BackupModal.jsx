...
export function BackupModal({ isOpen, onClose }) {
  ...
  const [isInitialLoad, setIsInitialLoad] = useState(true); // ✨ Флаг первого рендера
  ...
  const loadBackups = useCallback(async () => {
    ...
    try {
      ...
      showError('Не удалось загрузить список бэкапов');
    } finally {
      setLoading(false);
      // ✨ ИСПРАВЛЕНИЕ: Сбрасываем флаг первого рендера после загрузки
      if (isInitialLoad) {
        setIsInitialLoad(false);
      }
    }
  }, [showError, isInitialLoad]);
  ...
  const handleCreateBackup = async () => {
    ...
    try {
      ...
      showError('Ошибка при создании бэкапа');
    } finally {
      setIsCreating(false);
    }
  };
    ...
    restoreConfirm.openConfirm({
      ...
      onConfirm: async () => {
        ...
        try {
          ...
          showError('Ошибка при восстановлении данных');
        } finally {
    ...
    deleteConfirm.openConfirm({
      ...
      onConfirm: async () => {
        try {
          ...
          showError('Ошибка при удалении бэкапа');
        }
      }
    });
  };
  ...
  return (
    ...
    >
      ...
      {/* ✨ ИСПРАВЛЕНИЕ: Используем стабильный ключ для первого рендера */}
      <AnimatedModalContent contentKey={isInitialLoad ? 'initial' : `backups-${backups.length}`}>
      ...
      <div className="mt-4 pt-4 border-t border-gray-200 dark:border-gray-700">
        <p className="text-xs text-gray-500 dark:text-gray-400 text-center">
          Резервные копии автоматически создаются при изменениях данных.
          Хранится максимум 10 последних копий.
        </p>
      </div>
