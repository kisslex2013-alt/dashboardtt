...
export default defineConfig({
  plugins: [
    react(),
    // ✅ ОПТИМИЗАЦИЯ: Визуализация bundle для анализа размера
    visualizer({
  ...
  build: {
    ...
    rollupOptions: {
      output: {
        ...
        manualChunks(id) {
          ...
          // ✅ ОПТИМИЗАЦИЯ: Библиотеки для графиков (recharts и зависимости)
          if (id.includes('node_modules/recharts')) {
            return 'charts-vendor';
          }
          // ✅ ОПТИМИЗАЦИЯ: Иконки - самая большая библиотека, разделяем отдельно
          // Используем tree-shaking через named imports, поэтому lucide-react уже оптимизирован
          if (id.includes('node_modules/lucide-react')) {
            return 'icons-vendor';
          }
          // ✅ ОПТИМИЗАЦИЯ: Утилиты для работы с датами (date-fns)
          if (id.includes('node_modules/date-fns')) {
            return 'utils-vendor';
          }
          // ✅ ОПТИМИЗАЦИЯ: Tone.js для звуков (большая библиотека, загружается по требованию)
          if (id.includes('node_modules/tone')) {
            return 'audio-vendor';
          }
          // ✅ ОПТИМИЗАЦИЯ: Остальные node_modules в общий vendor chunk
    ...
    // ✅ ОПТИМИЗАЦИЯ: Увеличиваем лимит предупреждения о размере чанков (так как мы разделили на чанки)
    chunkSizeWarningLimit: 600,
    // ✅ ОПТИМИЗАЦИЯ: Включаем оптимизацию
    cssCodeSplit: true,
    reportCompressedSize: true,
    // ✅ ОПТИМИЗАЦИЯ: Включаем tree-shaking для удаления неиспользуемого кода
    treeshake: {
