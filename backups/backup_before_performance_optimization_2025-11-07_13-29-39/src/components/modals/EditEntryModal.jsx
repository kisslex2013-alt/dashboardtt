...
export function EditEntryModal({ isOpen, onClose, entry, onSave }) {
  ...
  // ИСПРАВЛЕНО: Сохраняем последний entry в ref, чтобы избежать мелькания title
  ...
  // ИСПРАВЛЕНО: Используем lastEntryRef для определения title, если модальное окно открыто
  // Если закрывается - используем сохранённое значение, чтобы избежать мелькания
  ...
  useEffect(() => {
    if (effectiveEntry) {
      ...
      if (effectiveEntry.categoryId && !effectiveEntry.category) {
        const foundCategory = categories.find(c => c.id === effectiveEntry.categoryId);
        ...
        const foundById = categories.find(c => c.id === effectiveEntry.category);
  ...
  // Валидация формы (используем улучшенную централизованную систему)
  const validateForm = () => {
  ...
  const handleSave = () => {
    ...
    // ИСПРАВЛЕНО: Находим ID категории по названию, если category - это название
    ...
    // ВАЖНО: Не используем ...formData, чтобы избежать копирования лишних полей (например, строкового earned)
    const saveData = {
      date: formData.date || effectiveEntry?.date || new Date().toISOString().split('T')[0],
      start: formData.start,
      end: formData.end,
      category: formData.category, // Название категории
      categoryId: categoryId, // ID категории
      description: formData.description || '',
      duration: parseFloat(duration),
      earned: earnedValue, // ИСПРАВЛЕНО: Используем числовое значение
    ...
    if (effectiveEntry?.id) {
      saveData.id = String(effectiveEntry.id); // ИСПРАВЛЕНО: Конвертируем id в строку для консистентности
  ...
  // ИСПРАВЛЕНО: Учитываем текущую редактируемую запись из формы, даже если она еще не сохранена
  const getDailyEarnings = () => {
    const dateToCheck = formData.date || effectiveEntry?.date || new Date().toISOString().split('T')[0];
    if (!dateToCheck) return 0;
    
    // Получаем все записи за день, исключая текущую редактируемую (если она уже сохранена)
    const dayEntries = entries.filter(e => 
      e.date === dateToCheck && 
      e.id !== effectiveEntry?.id // Исключаем текущую запись, если она уже в entries
    );
    ...
    // Это важно для новых записей и при редактировании существующих
  ...
  const getDateForEarnings = () => {
    return formData.date || effectiveEntry?.date || new Date().toISOString().split('T')[0];
  };
  
  // ИСПРАВЛЕНО: Используем effectiveEntry для определения title
  // Это предотвращает мелькание "Новая запись" при закрытии "Редактировать запись"
      ...
      <div className="space-y-4">
                    ...
                    if (value && formData.end) {
                      ...
                      if (startMinutes >= endMinutes) {
                        ...
                        const overlapError = checkTimeOverlap(
                          value, 
                          formData.end, 
                          formData.date || effectiveEntry?.date
                        );
                        if (overlapError) {
                          setErrors(prev => ({ 
                            ...prev, 
            ...
            <div className={`grid ${isMobile ? 'grid-cols-1' : 'grid-cols-2'} gap-4`}>
              <Input
                ...
                label="Заработок (₽)"
                type="number"
                value={formData.earned != null ? String(formData.earned) : ''} // ИСПРАВЛЕНО: Всегда строка для Input
                onChange={(value) => setFormData({ ...formData, earned: value })} // Сохраняем как строку, конвертируем при сохранении
                error={errors.earned}
