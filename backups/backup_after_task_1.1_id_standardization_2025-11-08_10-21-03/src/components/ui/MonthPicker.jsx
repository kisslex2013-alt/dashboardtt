...
export function MonthPicker({ value, onChange, onClose, inputRef }) {
  ...
  const [currentYear, setCurrentYear] = useState(value ? parseISO(value + '-01').getFullYear() : new Date().getFullYear());
  ...
  const pickerContent = (
    ...
    >
      ...
      <div className="flex items-center justify-between mb-3">
        ...
        <button
          onClick={handleToday}
          className="glass-button px-3 py-1.5 text-sm rounded-lg"
        >
          Сегодня
        </button>
        <button
      ...
      <div className="grid grid-cols-3 gap-2">
        {MONTH_NAMES.map((month, index) => {
          ...
          const isCurrentMonth = new Date().getFullYear() === currentYear && 
