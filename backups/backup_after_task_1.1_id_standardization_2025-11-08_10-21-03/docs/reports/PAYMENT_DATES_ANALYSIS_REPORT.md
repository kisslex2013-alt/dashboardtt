      ...
      {paymentData.map((payment, index) => (
        ...
        >
          <div className="flex justify-between items-baseline">
            <p className={`text-gray-500 dark:text-gray-400 text-sm ${shouldAnimate ? 'opacity-0 animate-fade-in' : ''}`}>
              {payment.name}{' '}
          ...
          <p className={`text-right text-xs text-gray-500 dark:text-gray-400 mt-1 ${shouldAnimate ? 'opacity-0 animate-fade-in' : ''}`}>
...
export function PaymentDatesSettings() {
  const { paymentDates, addPaymentDate, updatePaymentDate, deletePaymentDate, reorderPaymentDates } = useSettingsStore();
