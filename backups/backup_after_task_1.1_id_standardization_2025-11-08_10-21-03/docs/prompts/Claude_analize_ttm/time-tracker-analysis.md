...
const calculateEarnings = (entries) => {
  return entries.reduce((sum, entry) => 
    sum + (entry.duration * settings.hourlyRate), 0
  )
}
```
...
export const suggestNextTask = (currentTime, history) => {
  ...
  const similarTimeEntries = history.filter(e => 
    e.dayOfWeek === currentTime.dayOfWeek &&
    Math.abs(e.hour - currentTime.hour) < 1
  )
...
export const useBreakReminder = () => {
  ...
  useEffect(() => {
    const timer = setInterval(() => {
      ...
      if (workDuration >= 25 * 60) { // 25 минут
        setShouldBreak(true)
...
<style>
  ...
  body { margin: 0; font-family: system-ui; background: #111827; }
...
<motion.button
  whileTap={{ scale: 0.95 }}
  className={saved ? 'bg-green-500' : 'bg-blue-500'}
>
...
body {
  ...
  font-feature-settings: 
    'liga' 1, /* лигатуры */
    'calt' 1, /* контекстные альтернативы */
    'ss01' 1; /* стилистический сет */
