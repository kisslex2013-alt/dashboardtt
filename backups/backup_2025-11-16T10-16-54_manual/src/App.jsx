...
// AnalyticsSection имеет default export для lazy loading
const AnalyticsSection = lazy(() => import('./components/statistics/AnalyticsSection.jsx'))
  ...
  return (
    <>
      ...
      <main 
        ...
        tabIndex={-1}
      >
        {/* ✅ PERFORMANCE: Lazy loading для StatisticsOverview с Skeleton */}
        {/* ✅ ОПТИМИЗАЦИЯ: Откладываем загрузку статистики для улучшения LCP */}
        <Suspense fallback={<div className="mb-6"><SkeletonCard variant="statistic" /></div>}>
          <StatisticsOverview />
        </Suspense>
        
        {/* ✅ ОПТИМИЗАЦИЯ: Lazy loading для Pomodoro панели (если включена) */}
        {pomodoroSettings?.enabled && (
          <Suspense fallback={<div className="mb-6"><SkeletonCard variant="default" className="p-4" /></div>}>
            <PomodoroPanel />
          </Suspense>
        )}
        ...
        {/* ✅ ОПТИМИЗАЦИЯ: Lazy loading для EntriesList с Skeleton */}
        <Suspense fallback={<div className="glass-effect rounded-xl p-6"><SkeletonList count={3} variant="listItem" /></div>}>
          <EntriesList
