...
/**
 * Информация о месте в хранилище
 */
...
export class AppError extends Error {
  ...
  constructor(message: string, type: ErrorTypeValue = ErrorType.UNKNOWN, details: ErrorDetails = {}) {
    super(message)
    this.name = 'AppError'
    this.type = type
 ...
 */
export function handleError(error: Error | AppError, context: ErrorContext = {}): string {
...
export async function withErrorHandling<T>(
  ...
  try {
    ...
    const message = handleError(error as Error, { operation: operationName })
    throw new AppError(message, ErrorType.UNKNOWN, { originalError: error as Error })
  }
}
 ...
 */
export function tryCatch<T>(fn: () => T, operationName: string = 'Операция'): T {
  try {
    return fn()
  } catch (error) {
    const message = handleError(error as Error, { operation: operationName })
 ...
 */
export function checkStorageSpace(): StorageSpaceInfo {
  try {
 ...
 */
export function createStorageError(message?: string, details: ErrorDetails = {}): AppError {
