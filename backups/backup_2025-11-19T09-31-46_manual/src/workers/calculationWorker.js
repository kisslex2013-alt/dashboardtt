...
/**
 * Рассчитывает тренд заработка за последний месяц
 */
function calculateEarningsTrend(entries) {
 ...
 */
function calculateLongestSession(entries) {
  if (!entries || entries.length === 0) {
    return null
  }
...
self.onmessage = function (e) {
  ...
  try {
    ...
    switch (type) {
      ...
      case 'earningsTrend':
        result = calculateEarningsTrend(data)
        break

      case 'longestSession':
        result = calculateLongestSession(data)
        break
      ...
      case 'batch':
        ...
        result = {
          ...
          earningsTrend: calculateEarningsTrend(data),
          longestSession: calculateLongestSession(data),
        }
        break
