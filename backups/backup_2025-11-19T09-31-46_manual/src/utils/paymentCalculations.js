...
export function validatePaymentDate(payment, allPayments = []) {
  ...
  // Если указана конкретная дата, проверяем её формат
  if (payment.customDate && payment.customDate.trim() !== '') {
    ...
    // Проверка дня месяца (только если не указана конкретная дата)
  ...
  if (!payment.period || typeof payment.period !== 'object') {
    ...
    if (
      typeof payment.period.start !== 'number' ||
      payment.period.start < 1 ||
      payment.period.start > 31
    ) {
      errors.push('Начало периода должно быть от 1 до 31')
    }
    if (
      typeof payment.period.end !== 'number' ||
      payment.period.end < 1 ||
      payment.period.end > 31
    ) {
  ...
  if (payment.enabled !== false) {
    ...
    const hasOverlap = otherPayments.some(other => {
      ...
      if (other.monthOffset === payment.monthOffset) {
        ...
        return (
          (payment.period.start >= other.period.start &&
            payment.period.start <= other.period.end) ||
