...
import { subDays, format, eachDayOfInterval, parseISO, startOfMonth, endOfMonth, startOfYear, endOfYear, startOfDay } from 'date-fns';
 ...
 */
export function TrendsChart({ entries: entriesProp, dateFilter = 'month', customDateRange = { start: '', end: '' } }) {
      ...
      return (
        <div className={`glass-effect rounded-lg shadow-lg ${isMobile ? 'p-4' : 'p-3'}`}>
          ...
          {payload.map((item, index) => (
            <div key={index} className={`flex justify-between gap-4 ${isMobile ? 'text-base' : 'text-sm'}`}>
