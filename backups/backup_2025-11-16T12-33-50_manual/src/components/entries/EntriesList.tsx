...
const CategoriesModal = lazy(() =>
  import('../modals/CategoriesModal').then(module => ({ default: module.CategoriesModal }))
)
...
const BackupModal = lazy(() =>
  import('../modals/BackupModal').then(module => ({ default: module.BackupModal }))
)
...
export function EntriesList({
  ...
  // Слушаем события для открытия модалок из мобильного меню
  useEffect(() => {
    const handleOpenCategories = () => setIsCategoriesModalOpen(true)
    const handleOpenBackups = () => setIsBackupModalOpen(true)
    
    window.addEventListener('open-categories-modal', handleOpenCategories)
    window.addEventListener('open-backups-modal', handleOpenBackups)
    
    return () => {
      window.removeEventListener('open-categories-modal', handleOpenCategories)
      window.removeEventListener('open-backups-modal', handleOpenBackups)
    }
  }, [])
