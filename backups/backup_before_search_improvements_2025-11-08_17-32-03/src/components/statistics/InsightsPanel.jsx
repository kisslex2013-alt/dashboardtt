...
export function InsightsPanel({ shouldAnimate = true }) {
  ...
  const shouldUseWorker = entries && entries.length > 500;
  const { result: workerBestWeekday, isLoading: workerBestWeekdayLoading } = useWorkerCalculation(
    shouldUseWorker ? entries : [],
    'bestWeekday',
    'all'
  );
  const { result: workerPeakProductivity, isLoading: workerPeakProductivityLoading } = useWorkerCalculation(
    shouldUseWorker ? entries : [],
    'peakProductivity',
    'all'
  );
  ...
  const insights = useMemo(() => {
    ...
    if (shouldUseWorker && (workerBestWeekdayLoading || workerPeakProductivityLoading)) {
      return 'loading';
    }
    ...
    const bestDay = shouldUseWorker && workerBestWeekday 
      ? workerBestWeekday 
      : calculateBestWeekday(entries);
    insightsArray.push({
      id: 'best-weekday',
    ...
    const peak = shouldUseWorker && workerPeakProductivity
      ? workerPeakProductivity
      : calculatePeakProductivity(entries);
    insightsArray.push({
    ...
    insightsArray.push({
      ...
      description: (
        <>
          ...
          <span className={`font-bold ${trendHighlight} group-hover:drop-shadow-[0_2px_4px_rgba(0,0,0,0.4)] group-hover:brightness-110 transition-all duration-300`}>{trend.trend}</span>.
        </>
      ),
      icon: trendIcon,
  ...
  }, [entries, shouldUseWorker, workerBestWeekday, workerPeakProductivity, workerBestWeekdayLoading, workerPeakProductivityLoading]);
          ...
          <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
            {insights.map((insight, index) => (
              <div 
                ...
                style={shouldAnimate ? { animationDelay: `${0.1 + index * 0.05}s`, animationFillMode: 'both' } : {}}
              >
                <InsightCard
                  title={insight.title}
