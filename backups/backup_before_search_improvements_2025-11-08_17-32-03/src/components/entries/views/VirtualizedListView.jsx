...
export function VirtualizedListView({ entries, onEdit, selectionMode = false, selectedEntries = new Set(), onToggleSelection }) {
  ...
  const Row = useCallback(({ index, style, resizeMode, gridWidths, tableWidths, dragging, onDragStart, onDrag, onDragEnd }) => {
    // Защита от пустого массива или неверного индекса
    if (!groupedEntriesArray || index >= groupedEntriesArray.length || index < 0) {
      return null;
    }
      ...
      <div style={style}>
        ...
        >
          ...
          >
            ...
            {/* Содержимое summary */}
            <div className="relative px-3 py-2 grid grid-cols-[1fr_min-content_auto_auto] md:grid-cols-[1fr_min-content_auto_auto] items-center hover:bg-gray-50/50 dark:hover:bg-gray-800/50 transition-colors" style={{ columnGap: '2px' }}>
              {/* Левая часть: Дата и статус (вариант 5: min-content для процентов) */}
              ...
              {/* Столбец с процентами плана (вариант 5: min-content ширина) */}
              <div className="flex items-center gap-1.5 justify-center relative" style={{ marginLeft: `${gridWidths?.percentMargin || 2}px` }}>
              ...
              {/* Центр: Компактные инсайты (центрирование между процентами и итого) */}
              ...
              {/* Правая часть: Итого (центрирование инсайтов) */}
              <div className="flex items-center gap-1.5 justify-end relative" style={{ minWidth: '120px', marginLeft: `${gridWidths?.totalMargin || 80}px` }}>
          ...
          {isOpen && (
            <div className="border-t border-gray-200 dark:border-gray-700">
              <table className="w-full text-sm" style={{ tableLayout: 'fixed' }}>
                ...
                <colgroup>
                  {selectionMode && (
                    <col style={{ width: `${tableWidths?.checkbox || 40}px` }} />
                  )}
                  <col style={{ width: `${tableWidths?.time || 150}px` }} />
                  <col style={{ width: `${tableWidths?.category || 200}px` }} />
                  <col style={{ width: `${tableWidths?.hours || 80}px` }} />
                  <col style={{ width: `${tableWidths?.income || 100}px` }} />
                </colgroup>
                ...
                <tbody>
                  {sortedEntries.map((entry, entryIdx) => {
                    ...
                    return (
                      ...
                      >
                        {selectionMode && (
                          <td className="px-3 py-1.5 align-middle relative" style={{ verticalAlign: 'middle' }}>
                            <input
                              type="checkbox"
                        ...
                        <td className="px-3 py-1.5 align-middle font-mono text-xs text-gray-600 dark:text-gray-400 relative" style={{ verticalAlign: 'middle' }}>
                        ...
                        <td className="px-3 py-1.5 align-middle text-right text-xs text-gray-500 dark:text-gray-400 relative" style={{ verticalAlign: 'middle' }}>
                          {duration}ч
                          {/* Разделитель столбцов (только в режиме изменения, только в первой строке) */}
                          {resizeMode && entryIdx === 0 && (
                            <TableColumnDivider
                        ...
                        <td className="px-3 py-1.5 align-middle text-right font-semibold text-gray-800 dark:text-gray-200 relative" style={{ verticalAlign: 'middle' }}>
                          {earned}₽
                          {/* Разделитель столбцов (только в режиме изменения, только в первой строке) */}
                          {resizeMode && entryIdx === 0 && (
                            <TableColumnDivider
