  ...
  const handleSaveAsDefaults = () => {
    if (showConfirm) {
      ...
      if (success) {
        ...
        // Показываем уведомление об успехе с инструкциями
        showNotification(
          'Дефолтные значения сохранены! Проверьте консоль (F12) для инструкций по обновлению файла констант.',
          'success',
          8000
        );
      } else {
        showNotification(
          'Ошибка сохранения значений по умолчанию',
          'error',
          5000
        );
      }
    ...
    <>
      ...
      <div className="fixed top-4 right-4 flex items-center gap-2 z-[9999] animate-slide-down">
        ...
        <button
          ...
          title={showConfirm ? "Нажмите еще раз для подтверждения" : "Сохранить текущие значения как значения по умолчанию для всех пользователей"}
