...
export const EntryItem = memo(function EntryItem({ entry, onEdit }) {
  // Оптимизированные селекторы Zustand - только нужные данные
  ...
  const getCategoryName = (entry) => {
    ...
    if (entry.categoryId) {
      ...
      return category ? category.name : 'remix'; // Дефолт для undefined
    }
    
    // Если ничего нет - дефолт "remix" (для старых записей)
    return 'remix';
  };
