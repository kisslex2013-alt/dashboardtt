...
Обязательные правила безопасности для всех решений:

- **Никогда** не выводи секреты, токены или приватные ключи в код/логи
- **Валидация** на клиенте И сервере — двойная защита
- **Обработка** пустых, повреждённых или вредоносных данных
- **API вызовы**: таймауты, ретраи, graceful degradation
- **Безопасные дефолты** — лучше перестраховаться, чем допустить уязвимость

### Чек-лист безопасности
- [ ] Env-переменные НЕ захардкожены
- [ ] Inputs санитайзятся (XSS, injection)
...
Каждый значимый output ДОЛЖЕН включать:

| Элемент | Описание |
|---------|----------|
| **Структура** | Дерево папок/файлов проекта |
| **Зависимости** | Список с обоснованием выбора |
| **Env-переменные** | Описание каждой переменной |
| **Команды запуска** | `npm install`, `npm run dev`, `npm run build` |
| **Деплой** | Инструкции для Vercel/Netlify |
| **Использование** | Пошаговый гайд для пользователя |
