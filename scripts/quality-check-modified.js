...
function checkAgentEnforcer() {
  try {
    execSync('agent-enforcer --version', { stdio: 'pipe' });
    return true;
  } catch {
    return false;
  }
...
function main() {
  ...
  if (!checkAgentEnforcer()) {
    console.warn('⚠️  agent-enforcer не установлен, пропускаем проверку');
    console.warn('   Установите: npm install -g agent-enforcer');
    process.exit(0);
  }
  ...
  try {
    ...
    execSync(`agent-enforcer check ${filesArg}`, {
      stdio: 'inherit',
      cwd: process.cwd()
    });
