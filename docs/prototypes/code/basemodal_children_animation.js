 ...
 * ✅ Анимация при изменении размера окна браузера (window resize)
 * ✅ Анимация при изменении содержимого (children change) - НОВОЕ!
 */
...
export function BaseModal({
  ...
  // НОВОЕ: Ref для отслеживания предыдущего состояния children
  ...
  // НОВОЕ: Отслеживание изменений children через useLayoutEffect
  useLayoutEffect(() => {
    if (!isOpen || !panelRef.current || isExiting) return

    // Увеличиваем счетчик при каждом изменении children
    const newKey = childrenKeyRef.current + 1

    // Если это не первый рендер (счетчик > 0) и содержимое изменилось
    if (childrenKeyRef.current > 0) {
      // Сохраняем текущие размеры ДО обновления содержимого
      const savedWidth = dimensionsRef.current.width
      const savedHeight = dimensionsRef.current.height

      // Устанавливаем флаг изменения содержимого
      isContentChangingRef.current = true

      // Если есть сохраненные размеры, фиксируем их для начала анимации
      if (savedWidth > 0 && savedHeight > 0) {
        ...
        // Ждем, пока React отрендерит новое содержимое
        // Используем двойной RAF + небольшую задержку для гарантии
        requestAnimationFrame(() => {
          requestAnimationFrame(() => {
            setTimeout(() => {
              if (!panelRef.current) return

              // Измеряем новый размер после рендера нового содержимого
              const rect = panelRef.current.getBoundingClientRect()
              const newWidth = rect.width
              const newHeight = rect.height

              // Если размер действительно изменился, анимируем переход
              if (Math.abs(newWidth - savedWidth) > 1 || Math.abs(newHeight - savedHeight) > 1) {
                setDimensions({
                  width: `${newWidth}px`,
                  height: `${newHeight}px`,
                })
                ...
                // Размер не изменился, просто сбрасываем флаги
                setDimensions({ width: 'auto', height: 'auto' })
                setIsResizing(false)
                isContentChangingRef.current = false
              }
            }, 16) // ~1 кадр для гарантии применения стилей
          })
        })
      }
    }

    childrenKeyRef.current = newKey
  }, [children, isOpen, isExiting])

  // ResizeObserver для отслеживания изменений размера (window resize + children)
  useEffect(() => {
    ...
    const resizeObserver = new ResizeObserver(entries => {
      for (const entry of entries) {
        ...
        // ВАЖНО: Пропускаем обработку, если изменение вызвано children
        // (оно уже обрабатывается в useLayoutEffect выше)
        if (isContentChangingRef.current) {
          // Обновляем сохраненные размеры, но не запускаем анимацию
          dimensionsRef.current = { width, height }
          return
        }

        // Проверяем, действительно ли размеры изменились (window resize)
        if (
