...
export function VirtualizedListView({
        ...
        <div style={style}>
          ...
          >
            ...
            >
              ...
              {/* Содержимое summary */}
              <div
                className="relative px-3 py-2 grid grid-cols-[minmax(120px,1fr)_minmax(50px,min-content)_minmax(0,1fr)_minmax(100px,min-content)] md:grid-cols-[minmax(180px,1fr)_minmax(70px,min-content)_minmax(280px,1fr)_minmax(120px,min-content)] items-center hover:bg-gray-50/50 dark:hover:bg-gray-800/50 transition-colors"
                style={{ columnGap: '8px' }}
              >
                ...
                <div
                  className="flex items-center gap-1.5 justify-center relative flex-shrink-0"
                  style={{ marginLeft: `${gridWidths?.percentMargin || 2}px` }}
                >
                ...
                <div
                  ...
                  style={{ minWidth: '280px', marginLeft: `${gridWidths?.insightsMargin || 80}px` }}
                >
                  <span
                ...
                <div
                  className="flex items-center gap-1.5 justify-end relative min-w-0"
                  style={{ marginLeft: `${gridWidths?.totalMargin || 80}px` }}
                >
                  <span className="text-xs text-gray-500 dark:text-gray-400 whitespace-nowrap">
                    {metrics.totalHours.toFixed(2)}ч
                  </span>
                  <span
                    className={`text-lg font-bold whitespace-nowrap ${getStatusTextColor(metrics.status)}`}
                  >
                    {metrics.totalEarned}₽
                  </span>
                  {/* Разделитель столбцов (только в режиме изменения) */}
  ...
  const shouldVirtualize =
    (groupedEntriesArray && groupedEntriesArray.length > 100) || entries.length > 500
